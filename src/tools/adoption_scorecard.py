...
def _collect_critical_gaps(data: dict) -> list[dict[str, str]]:
    ...
    for step in remediation.get("steps", []):
        if step.get("priority") == "P0" and not any(
            g["gap"] == step.get("title") for g in gaps
        ):
            gaps.append({
                "gap": step["title"],
                "priority": "P0",
...
def _extract_days_to_green(data: dict) -> int:
    ...
    total_gaps = sum(
        data.get(k, {}).get("total_gaps", 0)
        for k in ("defender_coverage", "purview_policies", "entra_config")
    )
...
async def create_adoption_scorecard(assessment_context: str) -> dict[str, Any]:
    ...
    has_any_data = any(
        key in data
        for key in ("secure_score", "defender_coverage", "purview_policies", "entra_config")
    )
    ...
    # ── Compute overall ────────────────────────────────────
    known_workloads = [
        w for w in workload_status.values() if w["status"] != "unknown"
    ]
    if known_workloads:
        overall_pct = round(
            sum(w["coverage_pct"] for w in known_workloads) / len(known_workloads), 1
        )
    else:
        overall_pct = 0.0

    estimated_days = _extract_days_to_green(data)
    scorecard_md = _generate_markdown_scorecard(
        workload_status, top_5_gaps, overall_pct, estimated_days
    )
