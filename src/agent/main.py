...
def _tool_result(text: str) -> ToolResult:
    ...
    Applies PII redaction before passing tool output to the LLM,
    ensuring tenant GUIDs, emails, and IP addresses are not leaked
    into the model context.
    """
    return ToolResult(text_result_for_llm=redact_pii(text))
...
class SecPostureIQAgent:
    ...
    async def send_message(self, prompt: str) -> str | None:
        ...
        Applies RAI middleware pipeline:
          1. Input validation (length, char-set)
          2. Content safety check (blocks harmful input)
          3. Send to Copilot SDK
          4. Content safety check on output (blocks harmful responses)
        ...
        if not validation.is_valid:
            logger.warning("agent.input.rejected", reason=validation.reason)
            return f"⚠️ Input rejected: {validation.reason}"

        # ── RAI: Content safety on input ──────────────────────────
        input_safety = await check_content_safety(prompt, context="cli_input")
        if not input_safety["is_safe"]:
            logger.warning(
                "agent.input.content_safety_blocked",
                reason=input_safety["reason"],
                blocked=input_safety["blocked_categories"],
            )
            return (
                "⚠️ Your message was flagged by our content safety system. "
                "Please rephrase your question about your organization's security posture."
            )
        ...
        # ── RAI: Content safety on output ─────────────────────────
        if response_text:
            output_safety = await check_content_safety(response_text, context="cli_output")
            if not output_safety["is_safe"]:
                logger.warning(
                    "agent.output.content_safety_blocked",
                    reason=output_safety["reason"],
                    blocked=output_safety["blocked_categories"],
                )
                return (
                    "The generated response was blocked by our content safety system. "
                    "Please try again or rephrase your question. "
                    "⚠️ Generated by SecPostureIQ (AI-assisted)."
                )
