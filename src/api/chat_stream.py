...
async def stream_chat(
    ...
    # ── RAI: Content safety check on user input ─────────────────────
    input_safety = await check_content_safety(message, context="llm_input")
    if not input_safety["is_safe"]:
        logger.warning(
            "chat_stream.input_blocked",
    ...
    for _ in range(max_iterations):
        ...
        for tc_data in (tool_calls_map[i] for i in sorted(tool_calls_map)):
            ...
            # ── RAI: Redact PII from tool results before sending to LLM ──
            tool_result_json = json.dumps(result, default=str)
            redacted_result = redact_pii(tool_result_json)
    ...
    # ── RAI: Content safety check on LLM output ─────────────────────
    if full_content:
        output_safety = await check_content_safety(full_content, context="llm_output")
        if not output_safety["is_safe"]:
            logger.warning(
                "chat_stream.output_blocked",
