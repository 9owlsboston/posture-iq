...
def _format_scorecard(data: dict[str, Any]) -> str:
    ...
    if overall is not None:
        icon = "🟢" if overall >= 70 else ("🟡" if overall >= 40 else "🔴")
        lines.append(f"**Overall Adoption**: {icon} {overall:.0f}%\n")
    days = data.get("estimated_days_to_green")
    if days:
        lines.append(f"**Estimated Days to Green**: {days}\n")
    ...
    if workloads:
        ...
        for wl, info in workloads.items():
            ...
            # Sub-workload details if present
            sub = info.get("sub_workloads", {})
            for sw_name, sw_info in sub.items():
                sw_status = sw_info.get("status", "unknown")
                sw_icon = "🟢" if sw_status == "green" else ("🟡" if sw_status == "yellow" else "🔴")
                sw_pct = sw_info.get("coverage_pct", "?")
                lines.append(f"|   ↳ {sw_name} | {sw_icon} {sw_status} | {sw_pct}% |")
    top_gaps = data.get("top_5_gaps", [])
    if top_gaps:
        lines.append("\n### Top Gaps\n")
        for i, gap in enumerate(top_gaps, 1):
            pri = gap.get("priority", "")
            lines.append(f"{i}. **[{pri}]** {gap.get('gap', '?')} ({gap.get('workload', '?')})")
    disclaimer = data.get("disclaimer")
    if disclaimer:
        lines.append(f"\n> {disclaimer}")
    return "\n".join(lines)
...
def _format_playbook(data: dict[str, Any]) -> str:
    ...
    for area, pb in playbooks.items():
        ...
        if offer:
            ...
            if duration or delivery:
                lines.append(f"  Duration: {duration or 'N/A'} | Delivery: {delivery or 'N/A'}")
            lines.append("")
