...
# ═══════════════════════════════════════════════════════════════════════
# 8. Foundry IQ (Get to Green) Enrichment
# ═══════════════════════════════════════════════════════════════════════


class TestFoundryIQEnrichment:
    """Tests for _enrich_step_with_gtg_offer and _enrich_steps_with_gtg."""

    def test_step_with_legacy_auth_maps_to_conditional_access(self):
        from src.tools.remediation_plan import _enrich_step_with_gtg_offer

        step = _make_step(title="Block legacy authentication", description="Block legacy auth protocols")
        enriched = _enrich_step_with_gtg_offer(step)
        assert enriched["workload_area"] == "entra_conditional_access"
        assert "green_offer" in enriched
        assert enriched["green_offer"]["id"] == "GTG-EID-001"
    ...
    def test_step_with_safe_attachments_maps_to_defender_o365(self):
        ...
        assert enriched["green_offer"]["id"] == "GTG-DEF-002"
    ...
    def test_step_with_pim_maps_to_entra_pim(self):
        ...
        assert enriched["green_offer"]["id"] == "GTG-EID-002"
    ...
    def test_step_with_dlp_maps_to_purview_dlp(self):
        ...
        assert enriched["green_offer"]["id"] == "GTG-PUR-001"
    ...
    def test_step_with_identity_protection_maps_correctly(self):
        ...
        assert enriched["green_offer"]["id"] == "GTG-EID-003"
    ...
    def test_step_with_insider_risk_maps_correctly(self):
        ...
        assert enriched["green_offer"]["id"] == "GTG-PUR-004"
    ...
    def test_step_with_access_review_maps_correctly(self):
        ...
        assert enriched["green_offer"]["id"] == "GTG-EID-004"
    ...
    def test_unmatched_step_has_no_offer(self):
        ...
        enriched = _enrich_step_with_gtg_offer(step)
        assert "green_offer" not in enriched
    ...
    def test_enrich_steps_batch(self):
        from src.tools.remediation_plan import _enrich_steps_with_gtg
        ...
        enriched = _enrich_steps_with_gtg(steps)
        assert len(enriched) == 3
        # First two should have offers, third should not
        assert "green_offer" in enriched[0]
        assert "green_offer" in enriched[1]
        assert "green_offer" not in enriched[2]

    def test_enrich_does_not_mutate_original(self):
        from src.tools.remediation_plan import _enrich_steps_with_gtg

        original = _make_step(title="Block legacy auth")
        enriched = _enrich_steps_with_gtg([original])
        # Original should not be mutated (we dict-copy inside)
        assert "green_offer" not in original

    @pytest.mark.asyncio
    @patch("src.tools.remediation_plan._create_openai_client", return_value=None)
    async def test_mock_response_has_gtg_offers(self, _mock):
        """The mock fallback path should also produce GTG offers on steps."""
        from src.tools.remediation_plan import generate_remediation_plan

        result = await generate_remediation_plan("{}")
        steps_with_offer = [s for s in result["steps"] if "green_offer" in s]
        # Most mock steps should match a workload area
        assert len(steps_with_offer) >= 5, f"Expected >=5 steps with GTG offers, got {len(steps_with_offer)}"
